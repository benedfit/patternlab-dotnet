...
namespace PatternLab.Core.Engines
{
    ...
    public class RazorPatternEngine : IPatternEngine
    {
        ...
        public string Parse(Pattern pattern, Dictionary<string, object> data)
        {
            ...
            // Replace keys with hyphens in template
            var template = _replacedKeys.Aggregate(pattern.Html,
                (current, key) =>
                    current.Replace(key, key.Replace(PatternProvider.IdentifierSpace, PatternProvider.IdentifierHidden)));

            return Razor.Parse(template, model, pattern.Partial);
        }
        ...
        private void ParseKeys(IDictionary<string, object> dictionary)
        {
            ...
            foreach (
                var key in
                    dictionary.Keys.Where(key => key.Contains(PatternProvider.IdentifierSpace))
                        .Where(key => !_replacedKeys.Contains(key)))
            {
                // If key contains hyphen add it to the list of replacements
            ...
            foreach (var key in _replacedKeys)
            {
                ...
                var newKey = key.Replace(PatternProvider.IdentifierSpace, PatternProvider.IdentifierHidden);
